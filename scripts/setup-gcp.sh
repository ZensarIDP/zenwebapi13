...
if [ $# -lt 2 ]; then
    print_error "Usage: $0 <PROJECT_ID> <REGION> [SERVICE_ACCOUNT_NAME]"
    print_error "Example: $0 my-project-123 us-central1"
    exit 1
fi

PROJECT_ID=$1
REGION=$2
SERVICE_ACCOUNT_NAME=${3:-backstage-deployer}

print_step "Starting automated setup for project: $PROJECT_ID in region: $REGION"
...
if gcloud artifacts repositories describe docker-repo --location=$REGION --quiet >/dev/null 2>&1; then
    print_status "Repository 'docker-repo' already exists"
else
    print_status "Creating repository 'docker-repo'..."
...
echo "   ${REGION}-docker.pkg.dev/${PROJECT_ID}/docker-repo/[IMAGE_NAME]"
echo ""
